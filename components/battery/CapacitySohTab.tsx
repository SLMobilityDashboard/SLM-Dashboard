...
// -------------------- Color Generation --------------------
function generateTBoxColorMap(tboxIds: number[]): Record<number, string> {
  const colors = [
    "#10b981",
    "#3b82f6",
    "#f59e0b",
...
function processBatterySessionData(
  rawData: TBoxBMSSession[]
...
function getBatteryAnalytics(data: ProcessedBatterySession[]) {
  ...
  const allTBoxIds = [
    ...new Set(data.filter((d) => !d.isUnavailable).map((d) => d.tboxId)),
  ].sort((a, b) => a - b);

  const tboxColorMap = generateTBoxColorMap(allTBoxIds);
  ...
  allDatesInRange.forEach((date) => {
    ...
    daySessions.forEach((session) => {
      ...
      if (sessionDuration > 0) {
        ...
        if (session.isUnavailable) {
          ...
          tboxKey = session.tboxId.toString();
        }
    ...
    // Ensure we don't exceed 24 hours per day
    ...
    Object.entries(sessionMinutes).forEach(([tboxId, minutes]) => {
      tboxBreakdown[tboxId] = minutes / 60;
    });

    const activeHours = Object.entries(sessionMinutes)
      .filter(([tboxId]) => tboxId !== "UNAVAILABLE")
      .reduce((sum, [, minutes]) => sum + minutes / 60, 0);

    const unavailableHours = (sessionMinutes["UNAVAILABLE"] || 0) / 60;
    ...
    dailySummaries.push({
      ...
      uniqueTBoxes: new Set(
        daySessions.filter((d) => !d.isUnavailable).map((d) => d.tboxId)
      ).size,
      tboxBreakdown,
      totalHours: 24,
    });
...
const CustomVisxStackedTooltip = ({
  ...
  if (session.isUnavailable) {
    ...
    sessionColor = tboxColorMap[session.tboxId] || "#64748b";
    ...
    const fetchData = async () => {
      try {
        ...
        const sql = `WITH last_date AS (
                ...
                -- First part of gaps that cross midnight
                SELECT 
                    NULL AS SESSION_ID,
                    TBOXID,
                    BMSID,
                ...
                -- Second part of gaps that cross midnight
                SELECT 
                    NULL AS SESSION_ID,
                    TBOXID,
                    BMSID,
        ...
        setRawData(data);
        const processed = processBatterySessionData(data);
      ...
      <Card className="bg-slate-900/50 border-slate-800">
        <CardHeader>
          <CardTitle className="flex items-center gap-2 text-slate-100">
            ...
            Daily Battery Usage Timeline Last 90 days (By Vehicle)
          </CardTitle>
          <CardDescription className="text-slate-400">
            Battery sessions grouped by vehicle (TBox) in chronological order.
            Shows which vehicle used this battery each day. White = Not Attached
            to any vehicle. Each day sums to exactly 24 hours.
          </CardDescription>
        </CardHeader>
          ...
          <div style={{ position: "relative", height: 450, width: "100%" }}>
                        ...
                        <Group key={dayData.date} left={barX}>
                          {dayData.sessions.map((session, sessionIndex) => {
                            ...
                            if (session.isUnavailable) {
                              ...
                              sessionColor =
                                analytics.tboxColorMap[session.tboxId] ||
                                "#64748b";
                            }
            ...
            {tooltipData && (
              <CustomVisxStackedTooltip
                ...
                tboxColorMap={analytics.tboxColorMap}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card className="bg-slate-900/50 border-slate-800">
          <CardHeader>
            ...
            <CardDescription className="text-slate-400">
              Total hours this battery was attached to each vehicle
            </CardDescription>
          </CardHeader>
                  ...
                  data={(() => {
                    ...
                    analytics.dailySummaries.forEach((day) => {
                      Object.entries(day.tboxBreakdown).forEach(
                        ([tboxId, hours]) => {
                          tboxHours[tboxId] = (tboxHours[tboxId] || 0) + hours;
                        }
                      );
                    });
                    ...
                    return Object.entries(tboxHours)
                      .map(([tboxId, hours]) => {
                        ...
                        if (tboxId === "UNAVAILABLE") {
                          ...
                          displayName = "Not Attached";
                        } else {
                          color =
                            analytics.tboxColorMap[parseInt(tboxId)] ||
                            "#64748b";
                          displayName = `TBox ${tboxId}`;
                        }
        ...
        <Card className="bg-slate-900/50 border-slate-800">
          <CardHeader>
            ...
            <CardDescription className="text-slate-400">
              Battery attachment percentage per day - shows how actively this
