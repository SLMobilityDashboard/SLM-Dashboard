...
interface Props {
  ...
  loading: boolean;
  error: string | null;
  /** Refetch the underlying task-execution query (from useWarehouseQuery). */
  onRefetch?: () => void;
}
...
const TaskExecutionPanel = forwardRef<TaskExecutionPanelHandle, Props>(
  function TaskExecutionPanel({ data, loading, error, onRefetch }, ref) {
    const [statusFilter, setStatusFilter] = useState<string>("all");
    const [search, setSearch] = useState("");
    const { marked, toggle, refreshing, refresh } = useTaskMarks();
      ...
      <div className="space-y-4">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <button
            ...
            title="Refresh tasks and marks"
            className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium rounded-md border bg-slate-800/50 border-slate-700 text-slate-400 hover:text-slate-200 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
          >
