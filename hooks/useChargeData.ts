...
interface BatteryFilters {
  ...
  minSOH?: number;
  selectedVehicleImei?: string | null; // Selected vehicle IMEI
}
...
function useHomeCharging(
  ...
  const queries = useMemo(() => {
    // Use selectedVehicleImei if provided, otherwise fall back to tboxId
  ...
  }, [tboxId, filters.selectedVehicleImei, buildFilterConditions]);

  const loadData = useCallback(async () => {
    // Only load if we have a valid IMEI
    const effectiveTboxId = filters.selectedVehicleImei || tboxId;
    if (!effectiveTboxId) {
      setLoading(false);
      setBatteryData([]);
    ...
    try {
      ...
      console.log("Query" , queries.batteryTelemetry, "returned", telemetryResult.length, "rows");
      ...
      setDebugInfo({
        ...
        selectedVehicleImei: filters.selectedVehicleImei,
        effectiveTboxId,
  ...
  }, [loadData, tboxId, filters.selectedVehicleImei]);
...
export default useHomeCharging;