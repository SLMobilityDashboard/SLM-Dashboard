...
function getCacheStrategy(sql: string, forceDynamic?: boolean): {
  type: "static" | "daily" | "hourly" | "realtime";
  ttl:  number | null;
} {
  // forceDynamic means "this needs to be fresher than our SQL-shape-based
  // heuristics would guess" — so it should always win with the SHORTEST ttl,
  // never get overridden by (or override into) a longer bucket like daily.
  if (forceDynamic) return { type: "realtime", ttl: 30 };
