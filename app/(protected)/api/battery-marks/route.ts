// app/api/battery-marks/route.ts
//
...
export async function GET() {
  try {
    ...
    console.error("[battery-marks] GET failed:", err);
...
export async function POST(req: NextRequest) {
  try {
    ...
    const username = (session.user as any).username ?? session.user.email ?? "unknown";
    const redis = await getRedis();
    ...
    console.error("[battery-marks] POST failed:", err);
